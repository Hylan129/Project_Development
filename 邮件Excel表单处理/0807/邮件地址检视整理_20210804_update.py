...
root.title('邮件地址表单辅助处理-20210710_By Hylan')
					...
					if (huoyebo.ncols >= 5):
						for row,content_information in enumerate(zip(huoyebo.col_values(mail_col)[data_row:],huoyebo.col_values(cusname_col)[data_row:],huoyebo.col_values(mail_col+1)[data_row:],huoyebo.col_values(mail_col+2)[data_row:],huoyebo.col_values(mail_col+3)[data_row:])):
							try:
								...
								if str(mail_addr).strip() and str(customer_name).strip() and str(mail_addr).count("@") ==1:
									if mail_addr not in normal_data.keys():
										normal_data[mail_addr]=[1,[document,sheet,str(row + 1),customer_name]+list(content_information[2:])]
									else:
										normal_data[mail_addr].append([document,sheet,str(row+1),customer_name]+list(content_information[2:]))
										normal_data[mail_addr][0] += 1
									#可视化处理：
						...
						for row,content_information in enumerate(qingdan_data):
							try:
								...
								if str(mail_addr).strip() and str(customer_name).strip() and str(mail_addr).count("@") ==1:
									if mail_addr not in normal_data.keys():
										normal_data[mail_addr]=[1,[document,sheet,str(row + 1),str(customer_name).strip()]+list(content_information[2:])]
									else:
										normal_data[mail_addr].append([document,sheet,str(row+1),str(customer_name).strip()]+list(content_information[2:]))
										normal_data[mail_addr][0] += 1
									#可视化处理：
									#showinfo("数据整理：" + mail_addr+"地址发现" + str(alfa[mail_addr][0]) + "个")
									mail_ok += 1
				...
				if huoyebo.ncols >=5:
					for row,content_information in enumerate(zip(huoyebo.col_values(mail_col)[data_row:],huoyebo.col_values(cusname_col)[data_row:],huoyebo.col_values(mail_col+1)[data_row:],huoyebo.col_values(mail_col+2)[data_row:],huoyebo.col_values(mail_col+3)[data_row:])):
						...
						try:
							mail_addr,customer_name = content_information[0:2]
							if str(mail_addr).strip() and str(customer_name).strip() and str(mail_addr).count("@") ==1:
								if mail_addr not in alfa.keys():
									alfa[mail_addr]=[1,[document,sheet,str(row + 1),customer_name]+list(content_information[2:])]
								else:
									alfa[mail_addr].append([document,sheet,str(row+1),customer_name]+list(content_information[2:]))
									alfa[mail_addr][0] += 1
								#可视化处理：
								#showinfo("数据整理：" + mail_addr+"地址发现" + str(alfa[mail_addr][0]) + "个")
								mail_ok += 1
					...
					for row,content_information in enumerate(qingdan_data):
						...
						try:
							...
							if str(mail_addr).strip() and str(customer_name).strip() and str(mail_addr).count("@") ==1:
								if mail_addr not in alfa.keys():
									alfa[mail_addr]=[1,[document,sheet,str(row + 1),str(customer_name).strip()]+list(content_information[2:])]
								else:
									alfa[mail_addr].append([document,sheet,str(row+1),str(customer_name).strip()]+list(content_information[2:]))
									alfa[mail_addr][0] += 1
								#可视化处理：
								#showinfo("数据整理：" + mail_addr+"地址发现" + str(alfa[mail_addr][0]) + "个")
								mail_ok += 1
